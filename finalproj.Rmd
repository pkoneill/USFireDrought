...
## Data availability
  ...
  We chose to analyze two datasets because we are interested in both drought and wildfire trends and the relationship between the two, and we 
  found excellent data sources for each trend maintained separately. The two datasets chosen are detailed below:
  
  1. The [CDC United States Drought Monitor](https://data.cdc.gov/Environmental-Health-Toxicology/United-States-Drought-Monitor-2000-2016/spsk-9jj6) 
  reports weekly drought estimates at the county level throughout the contiguous United States. This data was collected by the Cooperative Institute for Climate and Satellites 
  in North Carolina, and is maintained by the CDC National Environmental Public Health Tracking Network to generate drought measures. The data spans a time period between 2000-2016 
  and was last updated November 27, 2018. Each week, each county in the United States, encoded by a combination of state ID and county ID or FIPS, has a recorded ordinal value 
  describing the drought status. The data is available in .csv format and contains 2.8 million rows - it can be read directly into R.
  ...
  2. [NFIRS Public Data](https://www.fema.gov/about/openfema/data-sets/fema-usfa-nfirs-annual-data) contains fire incident information across the US. The data is provided by the
   ...
   respond, including but not limited to fires. Each year, the data is released as a collection of separate text files delimited by the "^" character. The full "incidents" dataset 
   for any given year has around 2 million entries. Additional text files contain more detail about specific incidents including fires, hazardous materials etc. To analyze the data, 
   the text files for each year will have to be read into R, then combined into one DataFrame and filtered only to include wildfire-related incidents, then saved. In other words, 
   some pre-processing will be required and may have to be done locally due to the large file sizes. Since this data is recorded by fire department personnel, there is some 
...
map1$layers[[layer_no]]$default_aes$size = 2
map2$layers[[layer_no]]$default_aes$size = 2
...
fires_by_drought_condition[, c(1, 5, 6)] |> pivot_longer(
  ...
  scale_fill_discrete(
    labels = c('Percent of Wildfires', 'Percent of Time in Drought Condition')
  ) +
  theme_grey(12) + 
  theme(
    legend.position = 'top',
...
<iframe src="d3-viz.html" width="800" height="600"></iframe>
